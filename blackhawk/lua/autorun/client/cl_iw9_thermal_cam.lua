...
--
-- Deploy belly FLIR: free look, thermal grade, heat detection, hard-lock,
-- LRF, lead/rate HUD, and a designator laser along the sensor boresight.
-- Laser is networked and drawn only for seat 1 (pilot) and seat 2 (operator).
--
...
local LASER_MAX_UNITS     = 12000
local LASER_SEND_INTERVAL = 0.05
...
local MAT_LASER = Material("effects/laser1")
...
-- Remote designator state (received from co-pilot via server).
local remoteLaser = {
    active = false,
    vehicle = NULL,
    origin = Vector(),
...
local function ClearLock(playSound)
    ...
    cam.prevTgtAng = nil
    cam.prevTgtPos = nil
    if playSound and had then surface.PlaySound("buttons/button10.wav") end
end

local function ClearLRF()
    cam.lrfValid, cam.lrfUnits, cam.lrfMeters, cam.lrfFeet = false, 0, 0, 0
    cam.lrfTime, cam.lrfFlash = 0, 0
end
...
local function SendLaserOff()
    if not cam.laserWasOn then return end
    cam.laserWasOn = false
...
local function IsUnarmed(ply)
    ...
    if cls == "weapon_hands_sh" or cls == "weapon_hands" or cls == "weapon_fists" or cls == "none" then
        return true
    end
    return string.find(cls, "hands", 1, true) ~= nil
end

local function GetCharacterEyePos(character)
    if character:IsPlayer() then return character:EyePos() end
...
local function FindAimPlayer()
    ...
    local me = LocalPlayer()
    local tr = util.TraceLine({
        start = origin, endpos = origin + fwd * DETECT_RANGE,
        filter = { cam.vehicle, me }, mask = MASK_SHOT,
    })
    local hitPly = ResolvePlayerFromEntity(tr.Entity)
    if hitPly and hitPly ~= me and hitPly:Alive() and HasLOSToPlayer(hitPly) then return hitPly end
    ...
    for _, ply in player.Iterator() do
        ...
        if dot > bestDot then bestDot, best = dot, ply end
    end
    return best
end
...
local function SetLock(ply)
    ...
    cam.precYaw, cam.precPitch = 0, 0
    cam.joltYaw, cam.joltPitch = 0, 0
    cam.prevTgtAng, cam.prevTgtPos = nil, nil
    cam.tgtRateAz, cam.tgtRateEl = 0, 0
    cam.leadAz, cam.leadEl = 0, 0
...
local function ToggleLock()
    ...
    if IsValid(cam.lockTarget) then ClearLock(true) return end
    local tgt = FindAimPlayer()
    if tgt then SetLock(tgt) else surface.PlaySound("buttons/button8.wav") end
end

local function PulseLRF()
    if not IsSensorLive() then return false end
    local now = CurTime()
    if now < cam.lrfNext then surface.PlaySound("buttons/button10.wav") return true end
    local origin = GetSafeCameraOrigin(cam.vehicle)
    local ang = Angle(cam.pitch, cam.yaw, 0)
    if IsValid(cam.lockTarget) then
        local character = GetPlayerCharacter(cam.lockTarget)
        if IsValid(character) then ang = (GetCharacterEyePos(character) - origin):Angle() end
    end
    local me = LocalPlayer()
    local tr = util.TraceLine({
        start = origin, endpos = origin + ang:Forward() * LRF_MAX_UNITS,
        filter = { cam.vehicle, me }, mask = MASK_SHOT,
    })
    cam.lrfTime, cam.lrfNext, cam.lrfFlash = now, now + LRF_COOLDOWN, now + LRF_FLASH_TIME
    if tr.Hit and not tr.HitSky then
        local units = origin:Distance(tr.HitPos)
        cam.lrfValid, cam.lrfUnits = true, units
        cam.lrfMeters, cam.lrfFeet = units * UNITS_TO_METERS, units * UNITS_TO_FEET
...
local function GetLRFStatus()
    local now = CurTime()
    if now < cam.lrfFlash then return cam.lrfValid and "LRF FIRE" or "LRF NO RTN" end
    if now < cam.lrfNext then return "LRF HOLD" end
    return "LRF RDY"
end

-- ---------------------------------------------------------------------
-- Designator laser: local endpoints + net publish + draw
-- ---------------------------------------------------------------------

local function ComputeLocalLaser()
    if not IsSensorLive() or not IsValid(cam.vehicle) then return nil, nil end
    local origin = GetSafeCameraOrigin(cam.vehicle)
    local ang = Angle(cam.pitch, cam.yaw, 0)
    if IsValid(cam.lockTarget) then
        local character = GetPlayerCharacter(cam.lockTarget)
    ...
    local tr = util.TraceLine({
        start  = origin,
        endpos = origin + ang:Forward() * LASER_MAX_UNITS,
...
local function CanSeeDesignatorLaser(veh)
    if not IsValid(veh) then return false end
    local ply = LocalPlayer()
    if not IsValid(ply) then return false end
    if not ply.GlideGetVehicle or not ply.GlideGetSeatIndex then return false end
    if ply:GlideGetVehicle() ~= veh then return false end
    local seat = ply:GlideGetSeatIndex()
    return seat == PILOT_SEAT or seat == GIMBAL_SEAT
end

local function DrawDesignatorBeam(origin, hit)
    if not origin or not hit then return end
    render.SetMaterial(MAT_LASER)
    render.DrawBeam(origin, hit, 6, 0, 1, Color(255, 40, 30, 220))
    render.DrawBeam(origin, hit, 2, 0, 1, Color(255, 180, 160, 255))
    render.SetMaterial(MAT_GLOW)
    local sz = 24 + math.sin(RealTime() * 12) * 4
    render.DrawSprite(hit, sz, sz, Color(255, 60, 40, 200))
    render.DrawSprite(hit, sz * 0.45, sz * 0.45, Color(255, 220, 200, 255))
end

hook.Add("Think", "iw9_ThermalCam.LaserSend", function()
    if not IsSensorLive() then
        if cam.laserWasOn then SendLaserOff() end
        return
    end

    local now = CurTime()
    if now < cam.laserNextSend then return end
    cam.laserNextSend = now + LASER_SEND_INTERVAL

    local origin, hit = ComputeLocalLaser()
    if not origin then
        SendLaserOff()
        return
    end
...
net.Receive("iw9_ThermalCam.Laser", function()
    local veh = net.ReadEntity()
    local active = net.ReadBool()
    if not active then
        if remoteLaser.vehicle == veh then
            remoteLaser.active = false
            remoteLaser.vehicle = NULL
        end
        return
    end
    remoteLaser.active  = true
    remoteLaser.vehicle = veh
    remoteLaser.origin  = net.ReadVector()
    remoteLaser.hit     = net.ReadVector()
    remoteLaser.time    = CurTime()
end)

hook.Add("PostDrawTranslucentRenderables", "iw9_ThermalCam.LaserDraw", function(depth, sky)
    if depth or sky then return end

    -- Operator: draw from live local aim (no net lag).
    if IsSensorLive() and CanSeeDesignatorLaser(cam.vehicle) then
        local origin, hit = ComputeLocalLaser()
        DrawDesignatorBeam(origin, hit)
        return
    end

    -- Pilot (or co-pilot if local sensor not live): draw networked beam.
    if not remoteLaser.active then return end
    if CurTime() - remoteLaser.time > 0.35 then
        remoteLaser.active = false
        return
    end
    if not CanSeeDesignatorLaser(remoteLaser.vehicle) then return end
    DrawDesignatorBeam(remoteLaser.origin, remoteLaser.hit)
end)
...
hook.Add("PlayerBindPress", "iw9_ThermalCam.RMB", function(ply, bind, pressed)
    ...
    if bind == "+attack2" then cam.rmbHeld = pressed and true or false end
end)
...
hook.Add("PlayerBindPress", "iw9_ThermalCam.Toggle", function(ply, bind, pressed)
    ...
    if cam.deployState == "deploying" or cam.deployState == "stowing" then return true end
...
hook.Add("Think", "iw9_ThermalCam.Safety", function()
    if cam.active and not CanUseThermalCam() then StopThermalCam() end
    if cam.active and cam.rmbHeld and not input.IsMouseDown(MOUSE_RIGHT) then
        local ply = LocalPlayer()
        if not (IsValid(ply) and ply:KeyDown(IN_ATTACK2)) then cam.rmbHeld = false end
    end
    if IsSensorLive() and cam.lrfTime > 0 and not LRFHoldActive() then ClearLRF() end
end)
...
hook.Add("Think", "iw9_ThermalCam.LockTrack", function()
    ...
    if not IsValid(cam.lockTarget) then
        cam.lockTarget = NULL
        cam.tgtRateAz, cam.tgtRateEl, cam.tgtSpeedKts = 0, 0, 0
        cam.leadAz, cam.leadEl = 0, 0
        cam.prevTgtAng, cam.prevTgtPos = nil, nil
        return
    end
    local ply = cam.lockTarget
    if not ply:Alive() or not HasLOSToPlayer(ply) then ClearLock(true) return end
    local dt = FrameTime()
    if dt <= 0 then return end
    dt = math.min(dt, 0.05)
    local character = GetPlayerCharacter(ply)
    ...
    cam.cmdYaw, cam.cmdPitch = ang.y, math.Clamp(ang.p, -85, 85)
    if cam.prevTgtAng then
        cam.tgtRateAz = Lerp(math.min(1, 10 * dt), cam.tgtRateAz, AngleDiffDeg(ang.y, cam.prevTgtAng.y) / dt)
        cam.tgtRateEl = Lerp(math.min(1, 10 * dt), cam.tgtRateEl, (ang.p - cam.prevTgtAng.p) / dt)
    end
    cam.prevTgtAng = Angle(ang.p, ang.y, 0)
    local vel = ply:GetVelocity()
    if IsValid(character) and not character:IsPlayer() then
        if cam.prevTgtPos then
            cam.tgtSpeedKts = Lerp(math.min(1, 6 * dt), cam.tgtSpeedKts, (aimPos - cam.prevTgtPos):Length() / dt * 0.05)
        end
    else
        cam.tgtSpeedKts = Lerp(math.min(1, 6 * dt), cam.tgtSpeedKts, vel:Length() * 0.05)
    end
    cam.prevTgtPos = aimPos
    cam.leadAz, cam.leadEl = cam.tgtRateAz * LEAD_TIME, cam.tgtRateEl * LEAD_TIME
end)
...
hook.Add("InputMouseApply", "iw9_ThermalCam.Mouse", function(cmd, x, y, ang)
    ...
    if IsValid(cam.lockTarget) then return true end
...
hook.Add("Think", "iw9_ThermalCam.GimbalDynamics", function()
    ...
    if isLocked then maxRate = math.max(maxRate, 45) end
    if not IsDeployed() then
        local vehAng = vehicle:GetAngles()
        cam.cmdYaw, cam.cmdPitch = vehAng.y, 25
    end
    local errYaw   = AngleDiffDeg(cam.cmdYaw, cam.yaw)
    local errPitch = cam.cmdPitch - cam.pitch
    local twoZetaOmega, omegaSq = 2 * ZETA * OMEGA, OMEGA * OMEGA
    local accYaw   = math.Clamp(errYaw   * omegaSq - cam.rateYaw   * twoZetaOmega, -SLEW_ACCEL, SLEW_ACCEL)
    local accPitch = math.Clamp(errPitch * omegaSq - cam.ratePitch * twoZetaOmega, -SLEW_ACCEL, SLEW_ACCEL)
    ...
    if IsDeployed() and not isLocked then
        ...
        if slewMag > 1 then
            ...
            cam.precPitch, cam.precYaw = cam.precPitch * decay, cam.precYaw * decay
        end
        local precYawStep, precPitchStep = cam.precYaw * dt, cam.precPitch * dt
        ...
        if cam.prevVehAng then
            ...
            if math.abs(dYaw) > JOLT_THRESH then cam.joltYaw = cam.joltYaw + dYaw * JOLT_GAIN end
            if math.abs(dPitch) > JOLT_THRESH then cam.joltPitch = cam.joltPitch + dPitch * JOLT_GAIN end
            if math.abs(dRoll) > JOLT_THRESH then cam.roll = cam.roll + dRoll * JOLT_GAIN * 0.15 end
        end
        cam.prevVehAng = Angle(vehAng.p, vehAng.y, vehAng.r)
        local joltDecay = math.max(0, 1 - JOLT_DECAY * dt)
        cam.joltYaw, cam.joltPitch = cam.joltYaw * joltDecay, cam.joltPitch * joltDecay
    ...
    cam.roll = math.Clamp(Lerp(math.min(1, ROLL_RETURN * dt), cam.roll, -cam.rateYaw * ROLL_FROM_YAW), -5, 5)
...
hook.Add("Think", "iw9_ThermalCam.Zoom", function()
    ...
    cam.fov = Lerp(math.min(FrameTime() * 8, 1), cam.fov, wantZoom and FOV_MIN or FOV_BASE)
end)

hook.Add("PostPostHGCalcView", "iw9_ThermalCam.CalcView", function()
    if not cam.active or not CanUseThermalCam() then return end
    if cam.deployFrac <= 0.02 then return end
    local origin = GetSafeCameraOrigin(cam.vehicle) + Vector(0, 0, (1 - cam.deployFrac) * 18)
    local ang = Angle(cam.pitch + cam.shakeP, cam.yaw + cam.shakeY, cam.roll + cam.shakeR)
    return {
        origin = origin,
        angles = ang,
        fov = Lerp(cam.deployFrac, FOV_BASE + 20, cam.fov),
        drawviewer = true,
    }
end)
...
local thermalColorModify = {
    ...
    ["$pp_colour_brightness"] = 0, ["$pp_colour_contrast"] = 1.35, ["$pp_colour_colour"] = 0,
...
hook.Add("PreDrawHalos", "iw9_ThermalCam.DetectPeople", function()
    ...
    local origin, me = GetSafeCameraOrigin(cam.vehicle), LocalPlayer()
    for _, target in player.Iterator() do
        if target == me or not target:Alive() then continue end
        ...
        if tr.Fraction >= 0.98 then haloTargets[#haloTargets + 1] = character end
    end
...
hook.Add("HUDPaint", "iw9_ThermalCam.HUD", function()
    ...
    local sLeft, sRight = h * 0.1, w - (h * 0.1)
    ...
    if cam.deployState == "deploying" or cam.deployState == "stowing" then
        ...
        local bx, by = cx - barW / 2, cy + h * 0.06
    ...
    if isLocked then
        ...
        local lx, ly = cx + cam.leadAz * pxPerDeg, cy - cam.leadEl * pxPerDeg
        surface.SetDrawColor(lockCol)
        surface.DrawLine(lx - 6, ly, lx + 6, ly)
        surface.DrawLine(lx, ly - 6, lx, ly + 6)
    ...
    for degOffset = -45, 45 do
        if degOffset % 5 == 0 then
            local notchAng = math.floor(trueAzimuth / 5) * 5 + degOffset
            local tapeXPX = cx + math.AngleDifference(notchAng, trueAzimuth) * pxScaleMap
            if tapeXPX > bx and tapeXPX < bx + fW then
                ...
                if isMajor then
                    ...
                    elseif labelAngle == 270 then bearingStr = "W" end
    ...
    if lrfStatus == "LRF FIRE" then lrfStatusCol = lrfCol
    elseif lrfStatus == "LRF NO RTN" then lrfStatusCol = alertCol
    elseif lrfStatus == "LRF HOLD" then lrfStatusCol = Color(180, 180, 100, 230) end
    ...
    HDTS_Text("LSR  : DESIG ON", sLeft, sLeft + 45, nil, nil, Color(255, 80, 60, 230))
    ...
    HDTS_Text(string.format("COORDN : %06.0f", math.abs(pos.y * 11) % 100000), sRight, bHeight - 30, TEXT_ALIGN_RIGHT, nil, hudCol)
    HDTS_Text(string.format("COORDE : %06.0f", math.abs(pos.x * 11) % 100000), sRight, bHeight - 15, TEXT_ALIGN_RIGHT, nil, hudCol)
    ...
    if LRFHoldActive() and cam.lrfValid then
        ...
        HDTS_Text(string.format("R %05.0f M", cam.lrfMeters), cx, cy + h * 0.12, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, Color(lrfCol.r, lrfCol.g, lrfCol.b, alpha))
    ...
    if isLocked then
        ...
        local lx, ly, rx, ry = cx - gap, cy - gap, cx + gap, cy + gap
        ...
        if #onScreenSignatures > 0 then
            local fColor = (math.sin(RealTime() * 10) > 0) and alertCol or col
            HDTS_Text(#onScreenSignatures .. " TRK HEAT SGN ", cx, by + fH + 5, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, fColor)
            surface.SetDrawColor(alertCol)
            for i = 1, #onScreenSignatures do
                local sx, sy = onScreenSignatures[i].x, onScreenSignatures[i].y
                local sz, sl = 12, 4
