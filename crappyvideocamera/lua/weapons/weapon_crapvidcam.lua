...
hook.Add("ShouldDrawLocalPlayer", "CrapVidCam_DrawBody", function(ply)
    if VIDCAM_WRITER then
        return true
    end
end)

hook.Add("PreDrawViewModel", "CrapVidCam_RemoveVM", function(vm, ply)
    if VIDCAM_WRITER then
        return true
    end
end)
...
hook.Add("CalcView", "CrapVidCam", function(ply, pos, ang, fov)
    ...
    -- если мёртв — оставляем старую логику
    ...
    -- смещение назад и чуть вниз (видно тело)
    local offset = eyeAng:Forward() * -8 + eyeAng:Up() * -2

    return {
        origin = eyePos + offset,
        angles = eyeAng,
        fov = fov * VIDCAM_CVARS.FOV:GetFloat(),
        drawviewer = true -- рендерим модель игрока
    }
end)
