...
local function CheckAttachments(ply,tbl)
    ...
    for k,v in pairs(tbl.ABodygroups) do
        ...
        if not istable(tMdl) or not hg.Appearance.Bodygroups[k][tMdl.sex and 2 or 1] then continue end
...
local function ForceApplyAppearance(ply, tbl, noModelChange)
    ...
    if istable(tMdl) and tMdl.submatSlots then
        for k, v in pairs(tMdl.submatSlots) do
            local slot = 1
            for i = 1, #mats do
                if mats[i] == v then slot = i-1 break end
            end
            if hg.Appearance.Clothes and tbl.AClothes then
            ...
            ply:SetNWString("Colthes" .. k, (tbl.AClothes and tbl.AClothes[k]) or "normal")
        end
    end
    if hg.Appearance.FacemapsSlots and tbl.AFacemap then
        for i = 1, #mats do
    ...
    for k, v in ipairs(bodygroups) do
        ...
        if !hg.Appearance.Bodygroups or !hg.Appearance.Bodygroups[v.name] then continue end
        if not istable(tMdl) then continue end
        for i = 0, #v.submodels do
            local b = v.submodels[i]
            if !hg.Appearance.Bodygroups[v.name][tMdl.sex and 2 or 1] then continue end
    ...
    -- Tubbycity / external systems: bodygroups, skins, etc.
