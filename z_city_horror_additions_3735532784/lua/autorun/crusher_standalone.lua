...
-- Second grab: while already holding a primary victim, the crusher grabs
-- a second nearby player with his other hand. The primary grab carries
-- via hg.SetCarryEnt2 (the "carryent2" netvar family). cl_tpik.lua also
-- reads a fully parallel "carryent" family (carrybone/carrymass/carrypos,
-- no "2"), which strongly implies a sibling hg.SetCarryEnt for the other
-- hand slot -- but it's never actually called anywhere in this codebase,
-- so its existence is inferred from the netvar pairing, not confirmed.
-- If it doesn't exist, this will throw a clear "attempt to call a nil
-- value" error on grabbing a second victim -- test it in-game and report
-- back either way.
local function StopSecondGrab(ply)
    ...
    if SERVER then hg.SetCarryEnt(ply) end
...
local function StartSecondGrab(ply, victim)
    ...
    if SERVER then
        ...
        -- Mirrors TryCarry (below) left-right: that one offsets
        -- Vector(-2,0,0) / Right()*-5 / Angle(-90,90,0). This flips every
        -- one of those signs so it reads as the opposite hand instead of
        -- just re-grabbing the same side.
        local attempts = 0
        local function TryCarry2()
            if not IsValid(ply) or not IsValid(victim) then return end
            if not ply.Ability_SecondGrab or ply.Ability_SecondGrab.Victim ~= victim then return end
...
local function StopGrabbingHead(ply)
    ...
    if SERVER then
        if IsValid(victim) and victim.organism then
            -- Force the stand-back-up transition right now instead of
            -- waiting on a lightstun timer that's usually already lapsed
            -- by the time a grab actually ends (same reset the framework
            -- itself does on respawn, see sh_utility.lua). SetLocalVar is
            -- a server-only setter (every other call site in the base
            -- gamemode only ever calls it from SERVER), and this function
            -- runs on both realms, so this has to stay inside this block.
            victim.organism.lightstun = 0
            victim:SetLocalVar("stun", 0)
        end
        if IsValid(victim) then
            net.Start("HMCD_BeingVictimOfHeadGrab")
            net.WriteBool(false)
...
if SERVER then
    ...
    local CRUSHER_ESCAPE_STUN   = 1.5  -- how long the crusher is ragdolled for after losing their grip

    -- Shoves and ragdolls the crusher when a victim fights free -- a
    -- real consequence for holding on too long, not just an empty release.
    local function KnockBackCrusher(ply, victim)
        if not IsValid(ply) or not ply:IsPlayer() then return end

        hg.StunPlayer(ply, CRUSHER_ESCAPE_STUN)

        local pushDir = ply:GetPos() - victim:GetPos()
        pushDir.z = 0
        if pushDir:LengthSqr() < 1 then pushDir = -ply:GetAimVector() end
        pushDir:Normalize()
        local vel = pushDir * 260 + Vector(0, 0, 120)

        -- hg.StunPlayer spawns/attaches the ragdoll asynchronously (same
        -- caveat as the TryCarry loop above), so poll for it briefly
        -- rather than assuming it exists on the very next line.
        local attempts = 0
        local function TryKnockback()
            if not IsValid(ply) then return end
            ...
            attempts = attempts + 1
            if attempts < 20 then timer.Simple(0, TryKnockback) end
        end
        TryKnockback()
    end
        ...
        if grab_data and grab_data.Grabbed then
            ...
            if not stillHeld then
                ...
                if victim:Alive() and victim.organism then
                    ...
                    victim.organism.lightstun = CurTime() + 1
